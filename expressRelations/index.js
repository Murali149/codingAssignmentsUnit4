...
const connect = () => {
  return mongoose.connect(
    "mongodb+srv://Daniyal_12345@cluster0.tiemy.mongodb.net/database2?retryWrites=true&w=majority"
  );
};
...
app.get("/books", async (req, res) => {
  try {
    const book = await (
      await Book.find().populate({ path: "section_id", select: ["name"] })
    )
      .lean()
      .exec();
...
app.patch("/authors/:id", async (req, res) => {
  try {
    const author = await Author.findByIdAndUpdate(req.params.id, { new: true })
      .lean()
      .exec();
